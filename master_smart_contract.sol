...
contract MyToken is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    mapping(uint => address) public investor;
    uint public investor_count=0;
    ...
    function investor_transfer() public 
    {
        for(uint i=1;i<=investor_count;i++)
        {
            require(address(this).balance>=112*MINT_PRICE/100,"No ether left in the treasury");
            payable(investor[i]).transfer(112*MINT_PRICE/100);
        }
    }

     function amount_to_deposit() public view returns(uint) 
    {
        return address(this).balance*12/100;
    }
